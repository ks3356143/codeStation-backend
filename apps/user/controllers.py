...
class UserJWTController(TokenObtainPairController):
    ...
    # 分页获取用户数据
    @route.get(
        "/get_page_users", response = PaginatedResponseSchema[UserInfoOutSchema], url_name = 'user_page', summary = "分页获取用户"
    )
    ...
    def add_user(self, data: UserAddInputSchema):
        ...
        User.objects.create_user(username = extra_fields.pop("username"), password = extra_fields.pop("password"), **extra_fields)
