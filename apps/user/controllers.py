...
class UserJWTController(TokenObtainPairController):
    ...
    # 自定义obtain_token
    @route.post("/pair", url_name='token_obtain_pair')
    def obtain_token(self, user_token: schema.TokenObtainPairInputSchema):
        user = user_token._user
        # 这里验证用户是否启用
        if not user.enabled:  # type:ignore
            raise HttpError(405, "用户处于禁用状态，联系管理员后启用")
        refresh = RefreshToken.for_user(user)
        token = refresh.access_token  # type:ignore
        # 不使用默认输出，自定义输出
        return ChenResponse(code=200,
                            data={'username': user.username, 'access': str(token), 'refresh': str(refresh)})
    ...
    def modify_avatar(self, user_id: str, file: UploadedFile = File(...)):  # type: ignore
    ...
    # 分页获取用户数据
    @route.get(
        "/get_page_users", response=PaginatedResponseSchema[UserInfoOutSchema], url_name='user_page',
        summary="分页获取用户"
    )
    @paginate(PageNumberPaginationExtra, page_size=10)
    def get_page_users(self, filters: UserFilterSchema = Query(...)):  # type: ignore
    ...
    def add_user(self, data: UserAddInputSchema):
        ...
        User.objects.create_user(username=extra_fields.pop("username"), password=extra_fields.pop("password"),
                                 **extra_fields)
    ...
    def delete_admin(self, admin_id: str):
        ...
        # 判断是否为default_avatar
        if not admin.avatar == '/user_avatar/default.jpg':
            admin.avatar.delete()
        admin.delete()
